...
class LogGenerator:
    ...
    def write(self, dest, count):
        for i in range(count):
            ...
            msg = {'ip': ip, 'date': datetime.now(), 'request': request, 'resp_code': resp_code, 'resp_size': resp_size, 'ua': ua}
            dest.write(f'{json.dumps(msg, default=str)}\n')
            dest.flush()
...
qps = 4 if len(sys.argv) == 1 else int(sys.argv[1])
